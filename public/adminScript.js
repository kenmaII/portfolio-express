...
(function(){
  ...
  function renderProjects(list){
    ...
    list.forEach(p=>{
      ...
      div.innerHTML = `<strong>${escapeHtml(p.title||'Untitled')}</strong><div>${escapeHtml((p.description||'').slice(0,180))}</div><div style="margin-top:.4rem"><button class="edit">Edit</button> <button class="del" style="background:#ff6b6b">Delete</button></div>`;
      out.appendChild(div);
      div.querySelector('.edit').addEventListener('click', ()=> openEditor(p));
      div.querySelector('.del').addEventListener('click', async ()=>{
        if(!confirm('Delete project?')) return;
      try{ const r = await fetch('/api/projects/'+p._id,{ method:'DELETE', credentials:'include' }); const d = await r.json(); if(d.success){ siteAlert.show('Deleted'); loadProjects(); } else siteAlert.show('Delete failed','error'); }catch(e){ siteAlert.show('Network','error'); }
      });
    });
  }
  ...
  <div class="row"><label class="small">Project image (upload only)</label><input id="e_file" type="file" accept="image/*" class="input" style="width:auto"/><button id="e_upload" class="btn small">Upload</button><div class="preview" id="e_preview"></div></div>
  ...
  const payload = { title: e_title.value.trim(), imageUrl: (uploadedImageUrl||'').trim(), tags: (e_tags.value||'').split(',').map(s=>s.trim()).filter(Boolean), description: e_desc.value.trim() };
    if(!payload.title || !payload.description){ siteAlert.show('Title and description required','warn'); return; }
    try{
  if(editingId){ const r = await fetch('/api/projects/'+editingId,{ method:'PUT', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }); const dd = await r.json(); if(dd.success) siteAlert.show('Saved'); else siteAlert.show(dd.msg||'Save failed','error'); }
  else { const r = await fetch('/api/projects',{ method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }); const dd = await r.json(); if(dd.success) siteAlert.show('Added'); else siteAlert.show(dd.msg||'Add failed','error'); }
